...
Physics 
{
    ...
    (
        ...
        (
            Mobility
            (
                hSubBand(Doping EffectiveMass Scattering(MLDA) AutoOrientation RelChDir110)
                hSaturationFactor = 0.2
            )
...
Physics(Region="SiFilm") { hQuantumPotential(AutoOrientation Density) }

Physics ( Material = "Platinum" ) 
{
	MetalWorkfunction ( Workfunction = 4.65 )	# eV
}
...
*- Initial Solution
   ...
   Coupled(Iterations=100) { Poisson Hole hQuantumPotential }
...
*- AC analysis: Ramp Gate to Vgmax 
   ...
   Quasistationary(
      ...
      ){ Poisson Hole hQuantumPotential}
